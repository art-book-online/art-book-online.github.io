...
1. The ART Book exists because the authors wanted a book that taught, explained, and served as a reference of adaptive resonance theory algorithms for engineers and machine learning practitioners, and there wasn't one.
So they wrote one!
2. This website exists as a homepage for the book and a set of interactive exercises for the reader to get hands-on with the material, because that is the best way to learn machine learning material in general.
...
Because we are machine learning researchers and engineers!
And there isn't yet a book that, in one spot and succinctly, tells you:

1. What ART is and why it's important.
2. How ART machine learning models are derived from the theory.
3. How ART modules are adapted for supervised, unsupervised, and reinforcement learning.
4. A list (and implemented examples) of all of the ART modules that have been derived.
5. Pros and cons of each of those ART modules, and when to use (or not to use) them.

There exist books, papers, and literature reviews that do one or multiple of these very well, but there is none that does it all in one spot.
The ART Book is designed to fill that gap!

## The ART Book...Online

A book is all well and good, but what if you want to tinker with some code?
This is machine learning, after all, and the language of machine learning is software, code, and more code!
The book provides plenty of code examples for demonstrations of various concepts throughout the text (in addition to the code needed to generate its figures!), but what if you wanted to get hands-on without manually typing everything out yourself?

That is what this website is for!
Well, that, and as a homepage for hosting the book and providing updates on the books writing (and rewriting).
For interactive notebooks demonstrating various subject matter of the book, you can go to the [notebooks](/notebooks) section.
Feel free to look through them and even run them yourself!
For posts (such as this one) about various goings-on in the ART Book world, check out the [posts](/posts) section.

This book and its companion website are a passion project of the authors because of how much we believe in the material.
It is my sincere hope that you learn from, and maybe even enjoy a little bit, the ART Book and its companion website.
